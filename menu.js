...
(function(Menu) {
  ...
  Menu.version='1.528 ';//FOLDORUPDATEVERSION
  Menu.keys=keys;
    ...
    //      var cos=m.mctrl.cos;
      ...
      if (m.tf) {
        ...
        if (mco.co)
          ...
          +(mco.range?
            ' Animate <input type=checkbox oninput="Menu.mcontrol.animate=this.checked;"'+(mco.animate?' checked':'')+'/>'
            //+'<input type=text value="1x" size=3 />'
            +'<br><input id="menuRange" type="range" min="'+mco.range.min+'" max="'+mco.range.max+'"'
            +' style="width:300px;"'
            +' oninput='//mco==m.mctrl
        ...
        +(0&&(m.ms!==undefined)?'<div'+(m.msid?' id="'+m.msid+'"':'')+' style="font-size:'+mfs+'px'
        ...
        if (1&&(m.ms!==undefined)) {
          let d=document.createElement('div'),s=d.style;if (m.msid) d.is=m.msid;s.fontSize=mfs+'px';
          s.position='absolute';if (m.c2) { s.zIndex=10;s.textShadow='1px 0px 1px '+(m.tbgcol?m.tbgcol:'#aaa'); }
          s.top=((ch-mfs)*(m.mstop||1)-3)+'px';
          if (m.msCenter) { s.width='100%';s.textAlign='center'; } else s.left='2pt';
          d.innerHTML=m.ms;
          c.appendChild(d);
          m.msDiv=d;
        }
  ...
  Menu.ms=function(m,s) {
    m.ms=s;
    if (m.msDiv) m.msDiv.innerHTML=s;
    else if (m.msid) {
  ...
  Menu.touchStarts=function(e) {
    ...
    if (ret) {
      //onsole.log('Menu.touchStarts stopPropagation');
      e.preventDefault();
      e.stopPropagation();
      //onsole.log('propag stopped');
    }
  ...
  Menu.touchSticksRemove=function() {
    ...
    for (var i=0;i<tsd.length;i++) {
      var c=tsd[i];
      if (c.parentNode!==cont) continue;
      cont.removeChild(c);
      cont.removeChild(c.tc0);
      cont.removeChild(c.tc1);
    }
